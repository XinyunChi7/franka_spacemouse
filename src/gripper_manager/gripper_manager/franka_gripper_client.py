...
class GripperClient(Node):
  def __init__(self):
    ...
    self._gripper_command_subscription = self.create_subscription(Float32, gripper_command_topic, self._gripper_command_callback, 10)
    ...
    if not self._action_client.wait_for_server(timeout_sec=self._ACTION_SERVER_TIMEOUT):
      raise RuntimeError(f"Grasp action server not available after {self._ACTION_SERVER_TIMEOUT} seconds!")
  ...
  def _home_gripper(self, homing_action_topic: str) -> None:    
    ...
    if not homing_client.wait_for_server(timeout_sec=self._ACTION_SERVER_TIMEOUT):
      raise RuntimeError(f"Homing action server not available after {self._ACTION_SERVER_TIMEOUT} seconds!")
  ...
  def _get_max_gripper_width(self, joint_states_topic: str) -> None:
    ...
    gripper_subscription = self.create_subscription(JointState, joint_states_topic, joint_state_callback, 10)
